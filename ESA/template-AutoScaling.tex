...
\quantileRegression{ # TODO Define
iteratively re-weighted linear least squares to perform quantile 
regression
} 
least squres
\quantileRegression{
...
running time dataset with known scaling properties show that using the
...
point estimate (obtained by fitting a linear to the local data) less the 
...
and @@numInstsTest@@, respectively. The
...
@@perInstanceStatistic@@ running times.}}
\end{table*}
...
of $@@supportSizes@@$ variables, and are challenged by the @@statistic@@s of 
